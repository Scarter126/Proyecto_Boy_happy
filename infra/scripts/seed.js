...
profesoresCredenciales.forEach((prof, i) => {
  ...
  profesores.push({
    ...
    // NOTA: cursosAsignados eliminado - ahora se maneja en tabla ProfesorCurso
    _cursosAsignados: cursosAsignadosProfesores[i % cursosAsignadosProfesores.length], // Temporal para crear relaciones
...
// NOTA: Los apoderados ahora son entidades independientes en la tabla Apoderados
...
// Guardar metadata temporal para crear relaciones después
const alumnosRelaciones = [];
...
alumnosCredenciales.forEach((alumno, i) => {
  ...
  if (i < 6) {
    ...
    apoderado = apoderadosData[Math.floor(i / 2)];
  } else {
    // Últimos 4 alumnos: 1 hijo por apoderado
    apoderado = apoderadosData[3 + (i - 6)];
  }

  // Guardar relación temporal
  alumnosRelaciones.push({
    alumnoRut: alumno.rut,
    apoderadoRut: apoderado.rut,
    curso: curso.codigo,
    relacionParentesco: 'apoderado',
    esTitular: true
  });
  ...
  alumnos.push({
    ...
    rol: 'alumno',
    // NOTA: curso eliminado - ahora se maneja en tabla ApoderadoAlumno
...
const profesorCursoRelaciones = [];
profesores.forEach(profesor => {
  if (profesor._cursosAsignados) {
    profesor._cursosAsignados.forEach((curso, index) => {
      profesorCursoRelaciones.push({
        profesorRut: profesor.rut,
        cursoTipo: index === 0 ? `${curso}#jefe` : `${curso}#asignatura#${profesor.especialidad.split(',')[0].trim()}`,
        curso: curso,
        tipo: index === 0 ? 'jefe' : 'asignatura',
        asignatura: index === 0 ? null : profesor.especialidad.split(',')[0].trim(),
        activo: true,
        fechaAsignacion: profesor.fechaIngreso
      });
    });
    // Limpiar campo temporal
    delete profesor._cursosAsignados;
  }
});
...
alumnos.forEach((alumno, idx) => {
  ...
  comunicaciones.push({
    ...
    curso: alumnosRelaciones.find(r => r.alumnoRut === alumno.rut)?.curso,
    nombreApoderado: apoderadosData.find(a => a.rut === alumnosRelaciones.find(r => r.alumnoRut === alumno.rut)?.apoderadoRut)?.nombre + ' ' + apoderadosData.find(a => a.rut === alumnosRelaciones.find(r => r.alumnoRut === alumno.rut)?.apoderadoRut)?.apellido,
    telefonoApoderado: apoderadosData.find(a => a.rut === alumnosRelaciones.find(r => r.alumnoRut === alumno.rut)?.apoderadoRut)?.telefono,
    correoApoderado: apoderadosData.find(a => a.rut === alumnosRelaciones.find(r => r.alumnoRut === alumno.rut)?.apoderadoRut)?.correo,
    estado: estado,
...
// ===== NOTAS (~500 registros con evaluación conceptual para educación parvularia) =====
const asignaturas = ['Matemáticas', 'Lenguaje', 'Ciencias', 'Artes', 'Educación Física', 'Música'];
const nivelesLogro = ['L', 'OD', 'NL', 'NT']; // L=Logrado, OD=Objetivo en Desarrollo, NL=No Logrado, NT=No Trabajado
const evaluaciones = [
...
for (let i = 0; i < 500; i++) {
  ...
  recursosAcademicos.push({
    ...
    nombreEvaluacion: faker.helpers.arrayElement(evaluaciones),
...
async function seed() {
  ...
  try {
    ...
    await batchWriteToDynamoDB('Apoderados', apoderadosData);
    await batchWriteToDynamoDB('ApoderadoAlumno', alumnosRelaciones);
    await batchWriteToDynamoDB('ProfesorCurso', profesorCursoRelaciones);
    ...
    console.log(`   • ${apoderadosData.length} apoderados`);
    console.log(`   • ${alumnosRelaciones.length} relaciones apoderado-alumno`);
    console.log(`   • ${profesorCursoRelaciones.length} relaciones profesor-curso`);
