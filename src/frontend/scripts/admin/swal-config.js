...
// Wrapper para modales (NO toasts) con backdrop
const originalFire = SwalConfig.fire.bind(SwalConfig);
SwalConfig.fire = function(options) {
  // Si no es un toast, agregar backdrop
  if (!options.toast) {
    options.backdrop = options.backdrop !== undefined ? options.backdrop : 'rgba(0, 0, 0, 0.5)';
  }
