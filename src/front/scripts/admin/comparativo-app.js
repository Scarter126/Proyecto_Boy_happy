...
function comparativoApp() {
  return {
    ...
    async cargarDatos() {
      ...
      try {
        const [asistencia, notas, materiales] = await Promise.all([
          window.apiClient.get('/asistencia'),
          window.apiClient.get('/notas'),
          window.apiClient.get('/materiales')
        ]);

        this.asistencia = asistencia || [];
        this.notas = notas || [];
        this.materiales = materiales?.materiales || materiales || [];
        ...
        Notify.error('Error al cargar datos comparativos');
      } finally {
        this.loading = false;
      }
    },
    ...
    get datosComparativos() {
      ...
      return this.cursosSeleccionados.map(codigo => {
        ...
        notasCurso.forEach(n => {
          const nivel = n.nivel?.toLowerCase() || 'no trabajado';
          if (nivel.includes('logrado')) niveles.logrado++;
          else if (nivel.includes('desarrollo')) niveles.enDesarrollo++;
          else if (nivel.includes('no logrado')) niveles.noLogrado++;
          else niveles.noTrabajado++;
        });

        const promedio = notasCurso.length > 0
          ? ((niveles.logrado * 3 + niveles.enDesarrollo * 2 + niveles.noLogrado * 1) / notasCurso.length).toFixed(2)
          : 'N/A';
