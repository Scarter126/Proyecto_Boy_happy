...
// Niveles válidos
export const NIVELES_VALIDOS = ['L', 'NL', 'OD', 'NT'];
 ...
 */
export function nivelANota(nivelLogro) {
 ...
 */
export function calcularPromedioConceptual(notas) {
 ...
 */
export function calcularEstadisticasConceptuales(notas) {
  if (!Array.isArray(notas) || notas.length === 0) {
    return {
      total: 0,
      promedio: 0,
 ...
 */
export function determinarNivelDesempeno(promedio) {
 ...
 */
export function estaEnRiesgo(promedio, porcentajeAsistencia) {
 ...
 */
export function obtenerDescripcionNivel(nivelLogro) {
  const descripciones = {
